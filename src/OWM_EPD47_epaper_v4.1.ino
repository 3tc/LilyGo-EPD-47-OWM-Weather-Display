...
Forecast_record_type  LocalObservations[max_readings * 3];
int local_reading_count = 0;
float local_temp_readings[max_readings] = {0};
  ...
  if (StartWiFi() == WL_CONNECTED && SetupTime() == true) {
    ...
    if (WakeUp) {
      ...
      while ((RxWeather == false || RxForecast == false) && Attempts <= 2) { // Try up-to 2 time for Weather and Forecast data
        ...
        if (RxLocal == false) RxLocal = obtainLocalData(client);
        Attempts++;
      }
...
void DisplayWeather() { 
  ...
  DisplayForecastSection(260, 230); 
  DisplayGraphSection(5, 220);
}
...
void DisplayTempHumiPressSection(int x, int y) {
  ...
  if (WxConditions[0].Windspeed > 0) {
    drawString(x - 30, y + Yoffset, "Feels like " + String(WxConditions[0].FeelsLike, 1) + "° ", LEFT);   // Show FeelsLike temperature if windspeed > 0
    Yoffset += 30;
  }
  drawString(x - 30, y + Yoffset, String(WxConditions[0].High, 0) + "° / " + String(WxConditions[0].Low, 0) + "° H/L", LEFT); // Show forecast high and Low
}
...
void Display_UVIndexLevel(int x, int y, float UVI) {
  String Level = "";
  if (UVI <= 2)              Level = " (Low)";
  if (UVI >= 3 && UVI <= 5)  Level = " (Med)";
  if (UVI >= 6 && UVI <= 7)  Level = " (High)";
  if (UVI >= 8 && UVI <= 10) Level = " (Very High)";
  if (UVI >= 11)             Level = " (Extreme)";
...
String MoonPhase(int d, int m, int y, String hemisphere) {
  ...
  b   = int(b) & 7;                     /* 0 and 8 are the same phase so modulo 8 for 0 */
  if (hemisphere == "north") b = 7 - b;
...
int countReadings() {
  ...
  do {
    count++;
  } while (LocalObservations[count].Dt != 0);
  return count;
}
...
void DrawBattery(int x, int y) {
  ...
  if (voltage > 1 ) { // Only display if there is a valid reading
    ...
    drawString(x + 85, y - 14, String(percentage) + "% ", LEFT);
  }
}
...
#define number_of_dashes 20
  ...
  for (int i = 0; i < 3; i++) {
    drawString(20 + x_pos + gwidth / 3 * i, y_pos + gheight + 10, String(i - 2) + "d", LEFT);
...
bool obtainLocalData(WiFiClient & client) {
  client.stop(); // close connection before sending a new request
  HTTPClient http;
  String uri = "/v2/pws/observations/hourly/3day?apiKey=" + wundergroundKey + 
    "&stationId=" + wundergroundStationId + "&numericPrecision=decimal&format=json&units=m";
  http.begin(client, wunderground, 80, uri);
  int httpCode = http.GET();
  if (httpCode == HTTP_CODE_OK) {
    if (!DecodeLocal(http.getStream())) return false;
    client.stop();
  } else {
    Serial.printf("connection failed, error: %s", http.errorToString(httpCode).c_str());
    client.stop();
...
bool DecodeLocal(WiFiClient& json) {
  Serial.print(F("\nCreating local object...and "));
  DynamicJsonDocument doc(64 * 1024);                      // allocate the JsonDocument
  DeserializationError error = deserializeJson(doc, json); // Deserialize the JSON document
  if (error) {                                             // Test if parsing succeeds.
    Serial.print(F("DecodeLocal deserializeJson() failed: "));
    Serial.println(error.c_str());
    return false;
  }
  // convert it to a JsonObject
  JsonObject root = doc.as<JsonObject>();
  JsonArray list = root["observations"];
  byte row = 0;
  for (byte r = 0; r < max_readings * 3; r++) {
    if (r % 3 != 0 || list[r]["epoch"].as<int>() == 0) continue;
    Serial.println("\nPeriod-" + String(row) + "--------------");
    LocalObservations[row].Dt                = list[r]["epoch"].as<int>(); Serial.println("Time " + String(LocalObservations[row].Dt) + ": " + ConvertUnixTime(LocalObservations[row].Dt));
    LocalObservations[row].Temperature       = list[r]["metric"]["tempAvg"].as<float>();       Serial.println("Temp: " + String(LocalObservations[row].Temperature));
    LocalObservations[row].Low               = list[r]["metric"]["tempMin"].as<float>();       Serial.println("TLow: " + String(LocalObservations[row].Low));
    LocalObservations[row].High              = list[r]["metric"]["tempMax"].as<float>();       Serial.println("THig: " + String(LocalObservations[row].High));
    LocalObservations[row].Pressure          = list[r]["metric"]["pressureMax"].as<float>();   Serial.println("Pres: " + String(LocalObservations[row].Pressure));
    LocalObservations[row].Rainfall          = list[r]["metric"]["precipTotal"].as<float>();   Serial.println("Rain: " + String(LocalObservations[row].Rainfall));
    LocalObservations[row].Humidity          = list[r]["humidityAvg"].as<float>();             Serial.println("Humi: " + String(LocalObservations[row].Humidity));
    row += 1;
  }
  local_reading_count = row;
